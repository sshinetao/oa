...
namespace FTD.BLL
{
    ...
    public class ERPWorkRiZhi
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("UPDATE ERPWorkRiZhi SET haslook=ISNULL(haslook,0)+1 ");
            strSql.Append(" where ID=" + ID +"" );
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        public int Add()
        {
            ...
            strSql.Append("UserName,TitleStr,TimeStr,TypeStr,ContentStr,CanLookUser)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@CanLookUser", SqlDbType.Text)
                                        };
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,UserName,TitleStr,TimeStr,TypeStr,ContentStr,CanLookUser ");
        ...
        public DataSet getWeiTianRiZhiList()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM  ERPUser WHERE UserName <> 'admin' and UserName NOT IN ( ");
	        strSql.Append(" SELECT UserName FROM ERPWorkRiZhi ");
            strSql.Append(" where   TimeStr   BETWEEN convert(char(10),dateadd(dd, - DATEPART (weekday , getdate()+@@DATEFIRST -1)-6,getdate()),120) AND convert(char(10),dateadd(dd, - DATEPART (weekday , getdate()+@@DATEFIRST -1)+1,getdate()),120)  )");
            return DbHelperSQL.Query(strSql.ToString());
        }
