...
namespace FTD.BLL
{
	...
	public partial class SellOrder
	{
		...
		/// 得到一个对象实体
		/// </summary>
		public SellOrder(long Id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,number,title,Type,InvoiceType,InvoiceNum,QdTime,OrderAdd,LxAdd,Storeroom,ShTime,CompanyNumber,CompanyName,CompanyAdd,BankNumber,TaxNumber,Bank,Tel,LxPeople,DlPeople,Unit,StockPeoNum,StockPeoName,YxStarttime,YxEndtime,Discount,AllMoney,DisMoney,YjMoney,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,Gusername,Grealname,Gname,Sreakmart,SPUsername,SPRealname,MyTimes,Ifopenseller,storekeeper");
			strSql.Append(" FROM [SellOrder] ");
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                if (ds.Tables[0].Rows[0]["storekeeper"] != null)
                {
                    this.Storekeeper = ds.Tables[0].Rows[0]["storekeeper"].ToString();
                }
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [SellOrder]");
			strSql.Append(" where Id=@Id ");
		...
		public int Add()
		{
			...
			SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@seller", SqlDbType.NVarChar,50),
			...
			parameters[35].Value = Gusername;
			parameters[36].Value = Grealname;
			parameters[37].Value = Gname;
			parameters[38].Value = Sreakmart;
			parameters[39].Value = SPUsername;
			parameters[40].Value = SPRealname;
			parameters[41].Value = MyTimes;
			parameters[42].Value = Ifopen;
            parameters[43].Value = Seller;
            parameters[44].Value = Storekeeper;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [SellOrder] set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [SellOrder] ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.BigInt)};
			parameters[0].Value = Id;
