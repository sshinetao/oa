...
namespace FTD.BLL
{
	...
	public partial class ERPYS
	{
		...
		public ERPYS(int ID)
		{
			...
			strSql.Append("select ID,HeTongName,QianYueKeHu,DaoKuanDate,TiXingDate,CreateTime,CreateUser,BackInfo,NowState,HTJE,SFDK,FKID ");
			strSql.Append(" FROM [ERPYS] ");
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["FKID"]!=null && ds.Tables[0].Rows[0]["FKID"].ToString()!="")
				{
					this.FKID=int.Parse(ds.Tables[0].Rows[0]["FKID"].ToString());
				}
			}
		}
		...
		return DbHelperSQL.GetMaxID("ID", "ERPYS"); 
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [ERPYS]");
			strSql.Append(" where ID=@ID ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [ERPYS] (");
			strSql.Append("HeTongName,QianYueKeHu,DaoKuanDate,TiXingDate,CreateTime,CreateUser,BackInfo,NowState,HTJE,SFDK,FKID)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@FKID", SqlDbType.Int,4)};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [ERPYS] set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [ERPYS] ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
			parameters[0].Value = ID;
