...
namespace FTD.BLL
{
	...
	public partial class InSource
	{
		...
		/// 得到一个对象实体
		/// </summary>
		public InSource(long id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,Name,IfSystem ");
			strSql.Append(" FROM [InSource] ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [InSource]");
			strSql.Append(" where id=@id ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [InSource] (");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [InSource] set ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [InSource] ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.BigInt)};
			parameters[0].Value = id;
