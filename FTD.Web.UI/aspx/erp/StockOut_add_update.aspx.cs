...
{
	/// <summary>
	/// StockOut_add_update 的摘要说明。
	/// </summary>
	public class StockOut_add_update : System.Web.UI.Page
	{
		...
		//	Response.Write(SQL_GetList_drh);
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from StockOutMx   where id='" + int.Parse(Request.QueryString["id"])+"'";
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			string Sql_update ="Update StockOutMx  Set GoodsNumber='"+GoodsNumber.Text+"',GoodsName='"+GoodsName.Text+"',CountUnit='"+CountUnit.Text+"',Model='"+Model.Text+"',StockPoint='"+StockPoint.Text+"',SingleMoney='"+SingleMoney.Text+"',Allmoney='"+Allmoney.Text+"',Username='"+this.Session["Username"].ToString()+"',Realname='"+this.Session["TrueName"].ToString()+"' where id='" + int.Parse(Request.QueryString["id"])+"'";
