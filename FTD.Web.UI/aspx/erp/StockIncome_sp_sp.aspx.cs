...
{
	/// <summary>
	/// StockIncome_sp_sp 的摘要说明。
	/// </summary>
	public class StockIncome_sp_sp : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			//驳回
			string sql_down_bh="select top 1000 * from Cgrk_WorkStreamName where id<(select id from Cgrk_WorkStreamName where KeyFile='"+StreamNumber.Text+"' and Name_Jd='"+Gname.Text+"' and username='"+Gusername.Text+"') and KeyFile='"+StreamNumber.Text+"'";
		...
		{
				Response.Redirect("StockIncome_sp.aspx");
		}
		...
		private void ImageButton3_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			if(Ifts.SelectedValue=="否")
			{
				if(bhuser.SelectedValue=="启始位置")
				{
					...
					string sql_insert1="insert into StockIncomelog_lc  values('"+this.Session["TrueName"]+"将采购入库["+Name.Text+"]驳回给录入人','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
					List.ExeSql(sql_insert1);

					string Sql_update ="Update StockIncome  Set Gname='驳回审核',Grealname='驳回审核',Gusername='驳回审核',State='驳回审核',Sreakmart='"+Sreakmart.Text+"'    where id='" + int.Parse(Request.QueryString["id"])+"'";
				...
				else
				{
					...
					string    SQL_GetList     =  "select * from Cgrk_WorkStreamName  where  KeyFile='"+StreamNumber.Text+"' and Name_Jd='"+bhuser.SelectedItem.Text+"' and username='"+bhuser.SelectedValue+"'";
					OleDbDataReader NewReader = List.GetList(SQL_GetList);
					if(NewReader.Read())
					{
						string sql_insert1="insert into StockIncomelog_lc  values('"+this.Session["TrueName"]+"将采购入库["+Name.Text+"]驳回到节点"+bhuser.SelectedItem.Text+"','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
						List.ExeSql(sql_insert1);

						string Sql_update ="Update StockIncome  Set Gname='"+bhuser.SelectedItem.Text+"',Grealname='"+NewReader["realname"].ToString()+"',Gusername='"+bhuser.SelectedValue+"',Sreakmart='"+Sreakmart.Text+"'    where id='" + int.Parse(Request.QueryString["id"])+"'";
						List.ExeSql(Sql_update);
					}
					else
					{
				...
				string sql_insert=" insert into StockIncome_sp  (number,title,ShTime,StockPeoNum,StockPeoName,Storeroom,CompanyNumber,CompanyName,CompanyAdd,Traffic,Balance,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,SpCz,Sreakmart,Spusername,Sprealname,MyTimes) select number,title,ShTime,StockPeoNum,StockPeoName,Storeroom,CompanyNumber,CompanyName,CompanyAdd,Traffic,Balance,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,'驳回','"+Sreakmart.Text+"','"+this.Session["username"]+"','"+this.Session["TrueName"]+"',MyTimes from  StockIncome where  id='" + int.Parse(Request.QueryString["id"])+"'";
				List.ExeSql(sql_insert);
				
				//this.Response.Write("<script language=javascript>alert('提交成功！\n拒绝审批');window.location.href='StockIncome_sp.aspx'</script>");
				this.Response.Write("<script language=javascript>alert('提交成功！');window.location.href = 'StockIncome_sp.aspx'</script>");
			...
			else
			{
				...
				string    SQL_GetList     =  "select top 1 * from Cgrk_WorkStreamName where id>(select id from Cgrk_WorkStreamName where KeyFile='"+StreamNumber.Text+"' and Name_Jd='"+Gname.Text+"' and username='"+Gusername.Text+"') and KeyFile='"+StreamNumber.Text+"' order by id asc";
				OleDbDataReader NewReader = List.GetList(SQL_GetList);
				if(NewReader.Read())
				{
					string sql_insert1="insert into StockIncomelog_lc  values('"+this.Session["TrueName"]+"通过采购入库["+Name.Text+"]的审批，节点状态为："+NewReader["Name_Jd"].ToString()+"','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
					List.ExeSql(sql_insert1);

					string Sql_update ="Update StockIncome  Set Gname='"+NewReader["Name_Jd"].ToString()+"',Grealname='"+NewReader["realname"].ToString()+"',Gusername='"+NewReader["username"].ToString()+"' ,Sreakmart='"+Sreakmart.Text+"',State='正在审核'   where id='" + int.Parse(Request.QueryString["id"])+"'";
					List.ExeSql(Sql_update);
				...
				else
				{
					...
					string sql_insert1="insert into StockIncomelog_lc  values('"+this.Session["TrueName"]+"通过采购入库["+Name.Text+"]的审批，节点状态为：完成审核','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
					List.ExeSql(sql_insert1);



					string Sql_update ="Update StockIncome  Set Gname='完成审核',Grealname='完成审核',Gusername='完成审核',State='完成审核',Sreakmart='"+Sreakmart.Text+"'    where id='" + int.Parse(Request.QueryString["id"])+"'";
					List.ExeSql(Sql_update);

					string insert_xx="insert into nbdxx values('采购入库["+Name.Text+"]通过审批','采购入库["+Name.Text+"]通过审批\n通过时间："+System.DateTime.Now.ToString()+"','"+System.DateTime.Now.ToString()+"','"+username.Text+"','"+realname.Text+"','系统消息','系统消息','否','"+nbxx_num+"')";
					List.ExeSql(insert_xx);




					string    SQL_GetList_drh     =  "select * from StockIncomeMx   where Ifdd!='否' and Keyfile='"+number.Text+"'";
				
					OleDbDataReader NewReader_drh = List.GetList(SQL_GetList_drh);
					if(NewReader_drh.Read())
					{
						string Sql_update_drh ="Update StockOrder  Set State='已导入库单'  where number='" +NewReader_drh["Ifdd"]+"'";
					...
					string    SQL_GetList_rk     =  "select * from StockIncomeMx where  Keyfile='"+number.Text+"' ";
				...
				string sql_insert=" insert into StockIncome_sp  (number,title,ShTime,StockPeoNum,StockPeoName,Storeroom,CompanyNumber,CompanyName,CompanyAdd,Traffic,Balance,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,SpCz,Sreakmart,Spusername,Sprealname,MyTimes) select number,title,ShTime,StockPeoNum,StockPeoName,Storeroom,CompanyNumber,CompanyName,CompanyAdd,Traffic,Balance,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,'通过','"+Sreakmart.Text+"','"+this.Session["username"]+"','"+this.Session["TrueName"]+"',MyTimes from  StockIncome where  id='" + int.Parse(Request.QueryString["id"])+"'";
				List.ExeSql(sql_insert);

			this.Response.Write("<script language=javascript>alert('提交成功！商品成功入库');window.location.href = 'StockIncome_sp.aspx'</script>");
			}
