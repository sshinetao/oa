...
{
	/// <summary>
	/// OutRoom_update 的摘要说明。
	/// </summary>
	public class OutRoom_update : System.Web.UI.Page
	{
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			if(NewReader.Read())
			{
				...
				string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[出库]','出库','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
				List.ExeSql(sql_insert_xtrz);

			
				string sql_insert1="insert into OutRoomlog_lc values('"+this.Session["TrueName"]+"新增了出库["+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"]','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
				List.ExeSql(sql_insert1);


				string Sql_update ="Update OutRoom Set title='"+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',Storeroom='"+Storeroom.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',ThPeople='"+ThPeople.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',Tel='"+Tel.Text+"',ThTime='"+ThTime.Text+"',OutUse='"+OutUse.Text+"',Remark='"+Remark.Text+"',MyTimes='"+this.Session["MyTimes"]+"',StreamNumber='"+StreamNumber.Text+"',StreamName='"+StreamName.Text+"',Gusername='"+Gusername+"',Grealname='"+Grealname+"',Gname='"+Gname+"',Username='"+this.Session["Username"]+"',Realname='"+this.Session["TrueName"]+"',State='等待审批'  where  id='" + int.Parse(Request.QueryString["id"])+"'";
				
				List.ExeSql(Sql_update);

				this.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='OutRoom.aspx'</script>");
