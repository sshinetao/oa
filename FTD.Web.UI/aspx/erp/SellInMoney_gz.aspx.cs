...
{
	/// <summary>
	/// SellInMoney_gz 的摘要说明。
	/// </summary>
	public class SellInMoney_gz : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from SellInMoney   where id='" + int.Parse(Request.QueryString["id"])+"'";
			...
			string    SQL_Label2     =   "select  * from Xssk_WorkStreamName where KeyFile='"+StreamNumber.Text+"' order by id asc";
		...
		{
			Response.Redirect("SellInMoney_gz_rz.aspx?number="+Number.Text+"&id="+Request.QueryString["id"].ToString()+"");
		}
	}
}
