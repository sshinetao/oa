...
namespace qpsmartweb_jxc
{
	...
	/// IncomeRoom_sp 的摘要说明。
	/// </summary>
	public class IncomeRoom_sp : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
                string SQL_GetList_xs = "select * from IncomeRoom where   " + Server.UrlDecode(Request.QueryString["keywords"]) + "  like '%" + Server.UrlDecode(Request.QueryString["str"]) + "%' and CHARINDEX('" + this.Session["UserName"] + "',Gusername ) > 0  and (State!='驳回审核' and State!='完成审核' )    order by id desc";
			...
			{
				string SQL_GetList_xs    =  "select * from IncomeRoom  where  CHARINDEX('"+this.Session["UserName"]+"',Gusername ) > 0   and (State!='驳回审核' and State!='完成审核' )   order by id desc";
				Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
				Datagrid2.DataBind();
			}
		}
		...
		{
            Response.Redirect("IncomeRoom_sp.aspx?str=" + tilte.Text + "&keywords=" + ddl1.SelectedValue);
		}
		...
		private void Imagebutton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			try
			{
				...
				Response.Redirect("IncomeRoom_sp_sp.aspx?id="+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"");
