...
{
	/// <summary>
	/// StockIncome_add_add 的摘要说明。
	/// </summary>
	public class StockIncome_add_add : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				this.Response.Write("<script language=javascript>alert('该入库单已经导入订单，不能增加行！');window.close()</script>");
				return;
			}
		}
		...
		{
			string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[采购入库明细]','采购入库','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
			List.ExeSql(sql_insert_xtrz);

			string sql_insert="insert into StockIncomeMx (Keyfile,GoodsNumber,GoodsName,CountUnit,Model,StockPoint,SingleMoney,Allmoney,Username,Realname,Nowtimes,Ifdd) values('"+Request.QueryString["number"]+"','"+GoodsNumber.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"','"+GoodsName.Text+"','"+CountUnit.Text+"','"+Model.Text+"','"+StockPoint.Text+"','"+SingleMoney.Text+"','"+Allmoney.Text+"','"+this.Session["username"].ToString()+"','"+this.Session["TrueName"].ToString()+"','"+System.DateTime.Now.ToString()+"','否')";
