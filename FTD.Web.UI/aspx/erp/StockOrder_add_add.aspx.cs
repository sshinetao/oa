...
{
	/// <summary>
	/// StockOrder_add_add 的摘要说明。
	/// </summary>
	public class StockOrder_add_add : System.Web.UI.Page
	{
	...
	Db List=new Db();
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			string sql_insert="insert into StockOrderMx (Keyfile,GoodsNumber,GoodsName,CountUnit,Model,StockPoint,SingleMoney,Allmoney,Stocktype,Username,Realname,Nowtimes,YfMoney) values('"+Request.QueryString["number"]+"','"+GoodsNumber.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"','"+GoodsName.Text+"','"+CountUnit.Text+"','"+Model.Text+"','"+StockPoint.Text+"','"+SingleMoney.Text+"','"+Allmoney.Text+"','"+Stocktype.SelectedValue+"','"+this.Session["username"].ToString()+"','"+this.Session["TrueName"].ToString()+"','"+System.DateTime.Now.ToString()+"','0')";
