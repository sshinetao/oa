...
{
	/// <summary>
	/// StockOutMoney_gz 的摘要说明。
	/// </summary>
	public class StockOutMoney_gz : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from StockOutMoney   where id='" + int.Parse(Request.QueryString["id"])+"'";
			...
			string    SQL_Label2     =   "select  * from Cgfk_WorkStreamName where KeyFile='"+StreamNumber.Text+"' order by id asc";
			...
			while (NewReader_Label2.Read())
			{
				...
				if(NewReader_Label2["username"].ToString()==Gusername.Text && NewReader_Label2["Name_Jd"].ToString()==Gname.Text)
				{
		...
		{
			Response.Redirect("StockOutMoney.aspx");
		}

		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			Response.Redirect("StockOutMoney_gz_rz.aspx?number="+Number.Text+"&id="+Request.QueryString["id"].ToString()+"");
		}
	}
}
