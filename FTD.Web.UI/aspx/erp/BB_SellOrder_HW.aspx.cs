...
namespace qpsmartweb_jxc
{
	...
	public class BB_SellOrder_HW : System.Web.UI.Page
	{
		...
		public string CreateMidSql()
		{
			...
			{
				MidSql=MidSql+" and SellOrderMx.GoodsName like '%"+this.GoodsName.Text.Trim()+"%'";
			}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				string SQL_GetList_xs    =  "select SellOrderMx.* ,SellOrder.title from SellOrderMx,SellOrder where SellOrderMx.Keyfile=SellOrder.number "+Server.UrlDecode(Request.QueryString["str"])+" ";

				Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
				Datagrid2.DataBind();

				string    SQL_money_     =  " SELECT SUM(SellOrderMx.Allmoney) AS "+"sum"+"  from SellOrderMx ,SellOrder  where SellOrderMx.Keyfile=SellOrder.number  "+Server.UrlDecode(Request.QueryString["str"])+" ";
		...
		{
			Response.Redirect("BB_SellOrder_HW.aspx?str="+CreateMidSql()+"");
		}
