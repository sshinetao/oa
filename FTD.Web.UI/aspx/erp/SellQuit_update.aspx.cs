...
{
	/// <summary>
	/// SellQuit_update 的摘要说明。
	/// </summary>
	public class SellQuit_update : System.Web.UI.Page
	{
		...
		{
			string    SQL_GetList     =  "select top 1 * from Xsth_WorkStreamName  where KeyFile='"+StreamNumber.Text+"'";
			OleDbDataReader NewReader = List.GetList(SQL_GetList);
			if(NewReader.Read())
			{
				...
				string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[销售退货]','销售退货','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
				List.ExeSql(sql_insert_xtrz);

			
				string sql_insert1="insert into SellQuitlog_lc values('"+this.Session["TrueName"]+"新增了销售退货["+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"]','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
				List.ExeSql(sql_insert1);


				string Sql_update ="Update SellQuit Set title='"+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',ShTime='"+ShTime.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',Storeroom='"+Storeroom.Text+"',CompanyNumber='"+CompanyNumber.Text+"',CompanyName='"+CompanyName.Text+"',CompanyAdd='"+CompanyAdd.Text+"',StockPeoNum='"+StockPeoNum.Text+"',StockPeoName='"+StockPeoName.Text+"',Remark='"+Remark.Text+"',MyTimes='"+this.Session["MyTimes"]+"',StreamNumber='"+StreamNumber.Text+"',StreamName='"+StreamName.Text+"',Gusername='"+Gusername+"',Grealname='"+Grealname+"',Gname='"+Gname+"',Username='"+this.Session["Username"]+"',Realname='"+this.Session["TrueName"]+"',State='等待审批'  where  id='" + int.Parse(Request.QueryString["id"])+"'";
				
				List.ExeSql(Sql_update);

				this.Response.Write("<script language=javascript>alert('提交成功！');window.location.href='SellQuit.aspx'</script>");
		...
		private void ImageButton5_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			try
			{
				...
				string SQL_Del = "Delete from SellQuitMx where id='"+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"'";
		...
		private void Datagrid2_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			try
			{
				...
				ImageButton4.Attributes.Add("onclick","window.showModalDialog('SellQuit_add_update.aspx?a='+Math.random()+'&id="+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"&number="+number.Text+"','window','dialogWidth:700px;DialogHeight=400px;status:no;help=no;scroll=no');window.location='#'");
		...
		{
			Response.Redirect("SellQuit.aspx");
		}
	}
}
