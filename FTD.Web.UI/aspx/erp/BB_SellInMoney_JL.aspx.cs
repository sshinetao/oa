...
{
	/// <summary>
	/// BB_SellInMoney_JL 的摘要说明。
	/// </summary>
	public class BB_SellInMoney_JL : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				string SQL_GetList_xs    =  "select * from SellInMoney where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+" ";
				...
				string    SQL_money_1     =  " SELECT SUM(FkMoney) AS "+"sum"+" from SellInMoney where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+" ";
			...
			else
			{
				...
				string    SQL_money_1     =  " SELECT SUM(FkMoney) AS "+"sum"+" from SellInMoney";
				OleDbDataReader NewReader_money1 = List.GetList(SQL_money_1);
				if(NewReader_money1.Read())
				{
					try
		...
		{
			Response.Redirect("BB_SellInMoney_JL.aspx?str="+CreateMidSql()+"");
		}
		...
		private void Imagebutton3_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			try
			{
				...
				Response.Redirect("BB_SellInMoney_JL_show.aspx?id="+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"");
