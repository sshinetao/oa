...
{
	/// <summary>
	/// StockOrder_add 的摘要说明。
	/// </summary>
	public class StockOrder_add : System.Web.UI.Page
	{
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			if(NewReader.Read())
			{
				...
				string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[采购订单]','采购订单','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
				List.ExeSql(sql_insert_xtrz);

			
				string sql_insert1="insert into StockOrderlog_lc values('"+this.Session["TrueName"]+"新增了采购订单["+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"]','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
				List.ExeSql(sql_insert1);


				string sql_insert="insert into StockOrder  (number,title,Type,NbNumber,QdTime,OrderRemark,OrderAdd,LxAdd,Storeroom,ShTime,CompanyNumber,CompanyName,CompanyAdd,Unit,StockPeoNum,StockPeoName,AllMoney,YfMoney,State,Username,Realname,Nowtimes,StreamNumber,StreamName,Gusername,Grealname,Gname,MyTimes,Ifopen) values('"+number.Text+"','"+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"','"+Type.Text+"','"+NbNumber.Text+"','"+QdTime.Text+"','"+OrderRemark.Text+"','"+OrderAdd.Text+"','"+LxAdd.Text+"','"+Storeroom.Text+"','"+ShTime.Text+"','"+CompanyNumber.Text+"','"+CompanyName.Text+"','"+CompanyAdd.Text+"','"+Unit.Text+"','"+StockPeoNum.Text+"','"+StockPeoName.Text+"','"+AllMoney.Text+"','0','未执行','"+this.Session["username"].ToString()+"','"+this.Session["TrueName"].ToString()+"','"+System.DateTime.Now.ToString()+"','"+StreamNumber.Text+"','"+StreamName.Text+"','"+Gusername+"','"+Grealname+"','"+Gname+"','"+this.Session["MyTimes"]+"','"+Ifopen.SelectedValue+"')";
