...
{
	/// <summary>
	/// StockIncome_add 的摘要说明。
	/// </summary>
	public class StockIncome_add : System.Web.UI.Page
	{
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			if(NewReader.Read())
			{
				...
				string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[采购入库]','采购入库','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
				List.ExeSql(sql_insert_xtrz);

			
				string sql_insert1="insert into StockIncomelog_lc values('"+this.Session["TrueName"]+"新增了采购入库["+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"]','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
				List.ExeSql(sql_insert1);


				string sql_insert="insert into StockIncome  (number,title,ShTime,StockPeoNum,StockPeoName,Storeroom,CompanyNumber,CompanyName,CompanyAdd,Traffic,Balance,Remark,State,Username,Realname,Nowtimes,StreamNumber,StreamName,Gusername,Grealname,Gname,MyTimes) values('"+number.Text+"','"+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"','"+ShTime.Text+"','"+StockPeoNum.Text+"','"+StockPeoName.Text+"','"+Storeroom.Text+"','"+CompanyNumber.Text+"','"+CompanyName.Text+"','"+CompanyAdd.Text+"','"+Traffic.Text+"','"+Balance.Text+"','"+Remark.Text+"','等待审批','"+this.Session["username"].ToString()+"','"+this.Session["TrueName"].ToString()+"','"+System.DateTime.Now.ToString()+"','"+StreamNumber.Text+"','"+StreamName.Text+"','"+Gusername+"','"+Grealname+"','"+Gname+"','"+this.Session["MyTimes"]+"')";
