...
{
	/// <summary>
	/// GoodsSplit_add_update 的摘要说明。
	/// </summary>
	public class GoodsSplit_add_update : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from GoodsSplitLast   where id='" + int.Parse(Request.QueryString["id"])+"'";
		...
		{
			string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('修改[货物拆份明细]','货物拆份','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
			List.ExeSql(sql_insert_xtrz);

			string Sql_update ="Update GoodsSplitLast  Set GoodsNumber='"+GoodsNumber.Text+"',GoodsName='"+GoodsName.Text+"',CountUnit='"+CountUnit.Text+"',Model='"+Model.Text+"',Color='"+Color.Text+"',SplitPoint='"+SplitPoint.Text+"',Username='"+this.Session["Username"].ToString()+"',Realname='"+this.Session["TrueName"].ToString()+"' where id='" + int.Parse(Request.QueryString["id"])+"'";
