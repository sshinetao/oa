...
namespace qpsmartweb_jxc
{
	...
	/// OutRoom_ysp 的摘要说明。
	/// </summary>
	public class OutRoom_ysp : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				string SQL_GetList_xs    =  "select * from OutRoom_sp where  title like '%"+Server.UrlDecode(Request.QueryString["str"])+"%' and Spusername='"+this.Session["username"]+"'      order by id desc";
			...
			{
				string SQL_GetList_xs    =  "select * from OutRoom_sp  where  Spusername='"+this.Session["username"]+"'     order by id desc";
				Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
				Datagrid2.DataBind();
			}
		}
		...
		{
			Response.Redirect("OutRoom_sp.aspx");
		}
		...
		private void Imagebutton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			try
			{
				...
				Response.Redirect("OutRoom_ysp_show.aspx?id="+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"");
		...
		{
			Response.Redirect("OutRoom_ysp.aspx?str="+tilte.Text+"");
		}
		...
		private void ImageButton6_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			try
			{
				...
				string SQL_Del = "Delete from OutRoom_sp where id='"+Datagrid2.SelectedItem.Cells[1].Text.ToString()+"'";
				...
				if(Request.QueryString["str"]!=null)
				{
					string SQL_GetList_xs    =  "select * from OutRoom_sp where  title like '%"+Server.UrlDecode(Request.QueryString["str"])+"%' and Spusername='"+this.Session["username"]+"'  and (State!='驳回审核' and State!='完成审核' )    order by id desc";
					Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
					Datagrid2.DataBind();
				...
				{
					string SQL_GetList_xs    =  "select * from OutRoom_sp  where  Spusername='"+this.Session["username"]+"'   and  (State!='驳回审核' and State!='完成审核' )   order by id desc";
					Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
					Datagrid2.DataBind();
				}
