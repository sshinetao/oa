...
{
	/// <summary>
	/// SellQuit_add_update 的摘要说明。
	/// </summary>
	public class SellQuit_add_update : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			string    SQL_GetList_drh     =  "select * from StockOutMx   where Ifdd!='否' and Keyfile='"+Request.QueryString["number"]+"'";
			//	Response.Write(SQL_GetList_drh);
			OleDbDataReader NewReader_drh = List.GetList(SQL_GetList_drh);
			if(NewReader_drh.Read())
			{
				this.Response.Write("<script language=javascript>alert('该退货单已经导入订单，不能修改行！');window.close()</script>");
				return;
			}
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from SellQuitMx   where id='" + int.Parse(Request.QueryString["id"])+"'";
		...
		{
			string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('修改[采购退货明细]','采购退货','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
			List.ExeSql(sql_insert_xtrz);


			string Sql_update ="Update SellQuitMx  Set GoodsNumber='"+GoodsNumber.Text+"',GoodsName='"+GoodsName.Text+"',CountUnit='"+CountUnit.Text+"',Model='"+Model.Text+"',StockPoint='"+StockPoint.Text+"',SingleMoney='"+SingleMoney.Text+"',Allmoney='"+Allmoney.Text+"',Username='"+this.Session["Username"].ToString()+"',Realname='"+this.Session["TrueName"].ToString()+"' where id='" + int.Parse(Request.QueryString["id"])+"'";
