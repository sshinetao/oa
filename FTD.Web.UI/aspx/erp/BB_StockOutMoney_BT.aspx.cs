...
{
	/// <summary>
	/// BB_StockOutMoney_BT 的摘要说明。
	/// </summary>
	public class BB_StockOutMoney_BT : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				string SQL_GetList_xs    =  "select * from StockOrder where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+" ";
				Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
				Datagrid2.DataBind();

				string    SQL_money_     =  " SELECT SUM(ALLMoney) AS "+"sum"+" from StockOrder where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+" ";
				...
				string    SQL_money_1     =  " SELECT SUM(YfMoney) AS "+"sum"+" from StockOrder where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+" ";
			...
			else
			{
				...
				string    SQL_money_     =  " SELECT SUM(ALLMoney) AS "+"sum"+"  from StockOrder ";
				OleDbDataReader NewReader_money = List.GetList(SQL_money_);
				if(NewReader_money.Read())
				{
					try
				...
				string    SQL_money_1     =  " SELECT SUM(YfMoney) AS "+"sum"+" from StockOrder";
				OleDbDataReader NewReader_money1 = List.GetList(SQL_money_1);
				if(NewReader_money1.Read())
				{
					try
		...
		{
			Response.Redirect("BB_StockOutMoney_BT.aspx?str="+CreateMidSql()+"");
		}
