...
{
	/// <summary>
	/// StockOut_update 的摘要说明。
	/// </summary>
	public class StockOut_update : System.Web.UI.Page
	{
		...
		private void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
			...
			if(NewReader.Read())
			{
				...
				string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('修改[采购退货]','采购退货','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
				List.ExeSql(sql_insert_xtrz);

			
				string sql_insert1="insert into StockOutlog_lc values('"+this.Session["TrueName"]+"修改了采购退货["+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"]','"+System.DateTime.Now.ToString()+"','"+number.Text+"')";
				List.ExeSql(sql_insert1);



				string Sql_update ="Update StockOut Set title='"+title.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',ShTime='"+ShTime.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',StockPeoNum='"+StockPeoNum.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"',StockPeoName='"+StockPeoName.Text+"',Storeroom='"+Storeroom.Text+"',CompanyNumber='"+CompanyNumber.Text+"',CompanyName='"+CompanyName.Text+"',CompanyAdd='"+CompanyAdd.Text+"',Traffic='"+Traffic.Text+"',Balance='"+Balance.Text+"',Remark='"+Remark.Text+"',MyTimes='"+this.Session["MyTimes"]+"',StreamNumber='"+StreamNumber.Text+"',StreamName='"+StreamName.Text+"',Gusername='"+Gusername+"',Grealname='"+Grealname+"',Gname='"+Gname+"',Username='"+this.Session["Username"]+"',Realname='"+this.Session["TrueName"]+"',State='等待审批'  where  id='" + int.Parse(Request.QueryString["id"])+"'";
