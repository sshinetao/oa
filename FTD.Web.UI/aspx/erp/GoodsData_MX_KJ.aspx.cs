...
{
	/// <summary>
	/// GoodsData_MX_KJ 的摘要说明。
	/// </summary>
	public class GoodsData_MX_KJ : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				string SQL_GetList_xs    =  "select * from StoreAmount where 1=1 "+Server.UrlDecode(Request.QueryString["str"])+"  and (cast(Amount as decimal)<=Amountdown or cast(Amount as decimal)>=Amountup) order by id desc";
			...
			else
			{
				...
				string SQL_GetList_xs    =  "select * from StoreAmount where (cast(Amount as decimal)<=Amountdown or cast(Amount as decimal)>=Amountup)  order by id desc";
				Datagrid2.DataSource   = List.GetGrid_Pages(SQL_GetList_xs,"id");
				Datagrid2.DataBind();		
			}
		...
		{
			Response.Redirect("GoodsData_MX_KJ.aspx?str="+CreateMidSql()+"");
		}
