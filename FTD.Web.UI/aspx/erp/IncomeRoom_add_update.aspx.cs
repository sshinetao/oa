...
{
	/// <summary>
	/// IncomeRoom_add_update 的摘要说明。
	/// </summary>
	public class IncomeRoom_add_update : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from IncomeRoomMx   where id='" + int.Parse(Request.QueryString["id"])+"'";
		...
		{
			string Sql_update ="Update IncomeRoomMx  Set GoodsNumber='"+GoodsNumber.Text+"',GoodsName='"+GoodsName.Text+"',CountUnit='"+CountUnit.Text+"',Model='"+Model.Text+"',StockPoint='"+StockPoint.Text+"',Username='"+this.Session["UserName"].ToString()+"',Realname='"+this.Session["TrueName"].ToString()+"' where id='" + int.Parse(Request.QueryString["id"])+"'";
