<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="StockOrder_add_ncp.aspx.cs" Inherits="FTD.Web.UI.aspx.erp.StockOrder_add_ncp" %>
                                ...
                                <table cellspacing="1" cellpadding="4" width="100%" bgcolor="#d8e1e8" border="0">
                                    <tbody>
                                        <tr bgcolor="#f3f8fe">
                                            ...
                                            <td width="35%">
                                                <asp:DropDownList  ID="ddlOrderType" OnSelectedIndexChanged="ddlOrderType_SelectedIndexChanged" runat="server" Width="90%"></asp:DropDownList>
                                        ...
                                        <tr>
                                            <td width="100%" colspan="4" id="td1" style="display:none">
                             ...
                             onChange: function (n,o)
                              ...
                              var rowIndex = $('#dg').datagrid('getRowIndex',row);//获取行号                          
	                            ...
	                            row.countunit = '斤';  
                                                            ...
                                                            <th data-options="field:'laiyuancun',width:80,align:'left',editor:{type:'text'}">品名</th>
                                                            ...
                                                            <th data-options="field:'beizhu',width:250">备注</th>
                                                        </tr>
    ...
    <script type="text/javascript">
        ...
        $("#<%=ddlOrderType.ClientID %>").change(function () {
            ...
            if (dingdanleixing != "农产品") {
                location.href = '/aspx/erp/StockOrder_add_ncp.aspx'
            } else {
                location.href = '/aspx/erp/StockOrder_add.aspx'
            }
        })
        ...
        function CheckClientValidationState() {
            ...
            if (endEditing()) {
                ...
                for (var i = 0; i < rows.length; i++) {   // 每次保存一次 销售单内容
                    ...
                    zongjia = _All;
                    var ordernum = "";
                    var _json = jQuery.param({ "laiyuancun": row2.laiyuancun, "gname": row2.productname, "allprice": row2.allprice, "price": row2.price, "unitcost": row2.unitcost, "uname": uname, "rname": rname, 'number': number, 'itemid': row2.itemid });
                    var request = $.ajax({
                ...
                var _json3 = jQuery.param({ "title": title, "uname": uname, "rname": rname, "number": number, "ShTime": _shtime, 'CompanyAdd': _companyAdd, 'CompanyName': _CompanyName, 'Tel': _Tel, 'StockPeoNum': _StockPeoNum, 'StockPeoName': _StockPeoName, 'Discount': _Discount, 'AllMoney': zongjia, 'StreamNumber': _StreamNumber, 'Storeroom': _Storeroom });
                var request3 = $.ajax({
