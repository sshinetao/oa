...
{
	/// <summary>
	/// ReportSpill_add 的摘要说明。
	/// </summary>
	public class ReportSpill_add : System.Web.UI.Page
	{
		...
		{
			string sql_insert="insert into ReportSpill  (number,Storeroom,BsTimes,StockPeoNum,StockPeoName,Remark,Username,Realname,Nowtimes,MyTimes) values('"+number.Text+"','"+Storeroom.Text.Replace("<","〈").Replace(">","〉").Replace("'","’")+"','"+BsTimes.Text+"','"+StockPeoNum.Text+"','"+StockPeoName.Text+"','"+Remark.Text+"','"+this.Session["username"].ToString()+"','"+this.Session["TrueName"].ToString()+"','"+System.DateTime.Now.ToString()+"','"+this.Session["MyTimes"]+"')";
