...
{
	/// <summary>
	/// SellPlanYWY_add 的摘要说明。
	/// </summary>
	public class SellPlanYWY_add : System.Web.UI.Page
	{
		...
		{
			string sql_insert_xtrz="insert into SystemLog (Name,MkName,Username,Realname,Nowtimes) values ('新增[业务员销售计划]','业务员销售计划','"+this.Session["username"]+"','"+this.Session["TrueName"]+"','"+System.DateTime.Now.ToString()+"')";
			List.ExeSql(sql_insert_xtrz);

			string sql_insert="insert into SellPlanYWY   (number,SellsNumber,SellsName,Type,Limit,State,Starttime,Endtime) values('"+number.Text+"','"+SellsNumber.Text+"','"+SellsName.Text+"','"+Type.Text+"','"+Limit.Text+"','正常','"+Starttime.Text+"','"+Endtime.Text+"')";
