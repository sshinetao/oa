...
{
	/// <summary>
	/// Db_WorkStreamName 的摘要说明。
	/// </summary>
	public class Db_WorkStreamName : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				string    SQL_GetList     =  "select * from Db_WorkStreamType  where id='" + int.Parse(Request.QueryString["id"])+"'";
			...
			if (Request.QueryString["Del"]!= null)
			{
				try 
				{
					string SQL_Del = "Delete from Db_WorkStreamName where Id=" +Request.QueryString["Del"];
					List.ExeSql(SQL_Del);
			...
			string    SQL_Label2     =   "select  * from Db_WorkStreamName where KeyFile='"+KeyFile.Text+"' order by id asc";
		...
		{
			this.Response.Write("<script language=javascript>alert('设置成功！');window.location.href='Db_stream.aspx'</script>");
		}
	}
}
